#!/usr/bin/env bash
# LoRA SFT for expert B (B_sft.jsonl). Single GPU.
set -euo pipefail
...
OUT="${OUT:-${ROOT}/outputs/lora_B}"
DATA="${DATA:-${ROOT}/Data/Processed_data/B_sft.jsonl}"
GPU="${GPU:-0}"
...
echo "[run_lora_B] CUDA_VISIBLE_DEVICES=${GPU} BASE_MODEL=${BASE_MODEL}"
